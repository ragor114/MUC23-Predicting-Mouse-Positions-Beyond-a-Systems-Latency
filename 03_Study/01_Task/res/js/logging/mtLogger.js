...
class MTLogger {
    ...
    newTarget(width, length, buffer) {
        // console.log("Instance buffer: " + buffer);
        this.#currInstance = new MTInstance(width, length, buffer);
    }
    ...
    newTarget(width1, length, width2, reversed) {
        console.log("Instance without buffer");
        if(!reversed) {
    ...
    */
    ...
    toCSV() {
        let csvString = "compensation" + CSV_DELIMITER + "width" + Config.CSV_DELIMITER + "length" + Config.CSV_DELIMITER + "MT\n";
        this.#logList.forEach(element => {
            let w = element.width,
                l = element.length,
                mt = element.MT,
                compensation = Math.round((6 - element.buffer) * (1000/60)),
                w2,
                lineString = "" + compensation + CSV_DELIMITER + w + Config.CSV_DELIMITER + l + Config.CSV_DELIMITER + mt + "\n";
    ...
    lastInstanceToCSV() {
        let last = this.#logList[this.#logList.length-1],
            lineString = "",
            compensation = Math.round((6 - last.buffer) * (1000/60)),
            w = last.width,
            l = last.length,
            mt = last.MT,
            w2,
        ...
        lineString += compensation + Config.CSV_DELIMITER + wS + Config.CSV_DELIMITER + l + Config.CSV_DELIMITER + mt + "\n";
        
        return lineString;
    }

    get csvHeader() {
        const csvString = "compensation" + Config.CSV_DELIMITER + "width" + Config.CSV_DELIMITER + "length" + Config.CSV_DELIMITER + "MT\n";
        return csvString;
    }
...
class MTInstance {
    constructor(width, length, buffer, width2) {
        this.width = width;
        this.length = length;
        this.buffer = buffer;
        if(width2 != undefined) {
